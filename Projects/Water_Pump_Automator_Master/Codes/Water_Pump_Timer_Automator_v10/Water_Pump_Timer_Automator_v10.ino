...
#define POT_ENABLED 0           // enable potentiometer for changing off-time
#define LEVEL_SENSOR_ENABLED 0  // enable water level sensor

// --- setting for testing & troubleshooting

#define DEBUG_MODE_ENABLED 0      // display debug logs on serial monitor // DON'T FORGET TO DISABLE THIS BEFORE FINAL UPLOAD TO THE CARD
#define FAST_TEST_MODE_ENABLED 0  // 10 seconds on / 10 seconds off test mode // DON'T FORGET TO DISABLE THIS BEFORE FINAL UPLOAD TO THE CARD
...
// -- time-off options
constexpr uint32_t POT_CHECK_INTERVAL = 0.5 * CONVERSIONS::MINUTES_TO_MILLISECONDS;  // 1 minutes interval for reading potentiometer (300,000 ms)
...
constexpr uint8_t RELAY = 6;  // 6 IN UNO // CHANGED FROM 4 -> 3 -> 6
...
void loop() {
  ...
  if ((millis() - lastDebugTime) >= S_TIMERS_DELAYS::DEBUG_PERIOD_IN_S) {
    ...
    snprintf(buf_serial, sizeof(buf_serial),
             "Time: %lu s, Relay: %d, Level Sensor: %d-%d-%d, Pot: %d-%d, Time_off: %d min",
             (((millis() - S_TIMERS_DELAYS::STARTUP) / 1000)), !(digitalRead(PIN::RELAY)),
             LEVEL_SENSOR_ENABLED, digitalRead(PIN::LEVEL_SENSOR), digitalRead(PIN::LED_LEVEL_FULL),
             POT_ENABLED, analogRead(PIN::POT), (time_off / 60000));
...
#if (LEVEL_SENSOR_ENABLED)  // if level sensor is used
