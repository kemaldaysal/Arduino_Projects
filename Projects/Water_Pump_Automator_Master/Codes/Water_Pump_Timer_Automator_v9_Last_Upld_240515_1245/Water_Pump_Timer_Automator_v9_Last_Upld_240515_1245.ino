...
/*
  ...
  3) Disable edilmişlerse Lowpower, sleep gibi kütüphaneleri hiç include etme, ifndef gibi bir şey öğren
...
constexpr bool POT_ENABLED = false;           // enable potentiometer for changing off-time
constexpr bool LEVEL_SENSOR_ENABLED = false;  // enable water level sensor
constexpr bool SLEEP_OPTION_ENABLED = false;
// constexpr uint16_t water_level_threshold = 400; // only needed with analog sensor  // 430 -> 280 -> 265 -> 400 // don't go below 200 !!
}
...
constexpr uint8_t RELAY = 6;  // CHANGED FROM 4 -> 3 -> 6 İN CURRENT UNO!!
...
constexpr bool ON = 1; // IN CURRENT SYSTEM
constexpr bool OFF = 0; // IN CURRENT SYSTEM
}
...
//char buf_serial[BUF_SIZE]; // MOVED
//static volatile bool wakeUpRequested = false;  // top of file
...
void setup() {
  ...
  if (S_FEATURES::POT_ENABLED) {
    pinMode(PIN::POT, INPUT);
  }

  if (S_FEATURES::LEVEL_SENSOR_ENABLED) {
...
void loop() {
  ...
  */
  ...
  if (S_FEATURES::LEVEL_SENSOR_ENABLED && !(digitalRead(PIN::LEVEL_SENSOR)))  // if level sensor is used
  {
...
void WakeUp() {
  ...
  sleep_disable();
  detachInterrupt(digitalPinToInterrupt(PIN::LEVEL_SENSOR);
}*/
